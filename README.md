...
<img src="docs/assets/banner.svg" alt="LRIP — Lunar Resource Intelligence Platform" width="100%"/>
...
**LRIP occupies exactly the four gaps PRL 2026 leaves open.** It reproduces and extends the
...
Existing work — and most hackathon submissions — answer *"detect ice."* LRIP answers
*"run the mission."* The unit of output is not a CPR map; it is a **decision with a confidence level**.

| | Detection systems | **LRIP** |
|---|---|---|
...
interior — including warm, rocky terrain (the Fa & Eke false-positive problem). **Right:** LRIP's
physics-informed fusion collapses those false positives to a compact, thermally-stable ice lobe.

![Naive criterion vs LRIP calibrated P(ice)](docs/assets/money_shot.png)
...
**LRIP** (with an ice-confidence reward term that bends the path through the high-likelihood lobe).
...
the LRIP route dips to 29% in the permanently-shadowed segment, then recharges on exit.
...
- **Implementation:** [`ResourceIntelligence.tsx`](frontend/src/pages/ResourceIntelligence.tsx);
  ...
  the dielectric inversion is non-unique — LRIP provides it, **bounded honestly**.
...
Detection is the **input**. The Decision Layer is where LRIP becomes a planning system. Values
...
At runtime LRIP makes **no network calls** — all data is imported (PRD Rule 1). The platform is
...
| `GET /api/v1/traverse/{id}` | `TraversePlan` — LRIP + naive paths, ablation, SoC |
...
| Capability | Traditional detection workflow | **LRIP** |
|---|---|---|
...
| **Visualization** | CPR `#ff6b35` · DOP `#4ecdc4` · P(ice) `#cc66ff` · LRIP path `#00e5ff` · naive `#ff7043` · volume `#3366ff` |
...
- **PRL Ahmedabad** — Sinha, Bharti et al. (2026), the detection result LRIP extends.
...
```bibtex
@software{lrip2026,
  title  = {LRIP: Lunar Resource Intelligence Platform},
  author = {LRIP Team},
  year   = {2026},
...
<div align="center">
<br/>
<sub><b>LRIP</b> — Lunar Resource Intelligence Platform · Detection → Decision → Traverse → Confidence</sub>
</div>
